...
{
    SDL_GPUCopyPass *copy_pass = static_cast<SDL_GPUCopyPass *>(check_error_ptr(SDL_BeginGPUCopyPass(context->cmd_buf)));
    const SDL_GPUTransferBufferLocation vertex_transfer_buffer_location = SDL_GPUTransferBufferLocation {
        .transfer_buffer = context->transfer_buffer,
        .offset = 0
    };
    const SDL_GPUBufferRegion vertex_buffer_region = SDL_GPUBufferRegion {
    ...
    SDL_UploadToGPUBuffer(
        ...
        false);
    const SDL_GPUTransferBufferLocation index_transfer_buffer_location = SDL_GPUTransferBufferLocation {
        .transfer_buffer = context->transfer_buffer,
        .offset = sizeof(Vertex) * MAX_VERTEX_COUNT
    };
    const SDL_GPUBufferRegion index_buffer_region = SDL_GPUBufferRegion {
...
int main(int argc, char* argv[])
{
    ...
    context.window = static_cast<SDL_Window *>(check_error_ptr(SDL_CreateWindow("GPU text test", 800, 600, 0)));

    context.device = static_cast<SDL_GPUDevice *>(check_error_ptr(SDL_CreateGPUDevice(SUPPORTED_SHADER_FORMATS, true, NULL)));
    check_error_bool(SDL_ClaimWindowForGPUDevice(context.device, context.window));

    SDL_GPUShader *vertex_shader = static_cast<SDL_GPUShader *>(check_error_ptr(load_shader(context.device, VertexShader, 0, 1, 0, 0)));
    SDL_GPUShader *fragment_shader = static_cast<SDL_GPUShader *>(check_error_ptr(load_shader(context.device, use_SDF ? PixelShader_SDF : PixelShader, 1, 0, 0, 0)));
    ...
    SDL_GPUColorTargetDescription color_target_descriptions[] = { color_target_desc };
    ...
    context.pipeline = static_cast<SDL_GPUGraphicsPipeline *>(check_error_ptr(SDL_CreateGPUGraphicsPipeline(context.device, &pipeline_create_info)));
    ...
    context.vertex_buffer = static_cast<SDL_GPUBuffer *>(check_error_ptr(SDL_CreateGPUBuffer(context.device, &vbf_info)));

    SDL_GPUBufferCreateInfo ibf_info = {
        .usage = SDL_GPU_BUFFERUSAGE_INDEX,
        .size = sizeof(int) * MAX_INDEX_COUNT
    };
    context.index_buffer = static_cast<SDL_GPUBuffer *>(check_error_ptr(SDL_CreateGPUBuffer(context.device, &ibf_info)));

    SDL_GPUTransferBufferCreateInfo tbf_info = {
        .usage = SDL_GPU_TRANSFERBUFFERUSAGE_UPLOAD,
        .size = (sizeof(Vertex) * MAX_VERTEX_COUNT) + (sizeof(int) * MAX_INDEX_COUNT)
    };
    context.transfer_buffer = static_cast<SDL_GPUTransferBuffer *>(check_error_ptr(SDL_CreateGPUTransferBuffer(context.device, &tbf_info)));
    ...
    context.sampler = static_cast<SDL_GPUSampler *>(check_error_ptr(SDL_CreateGPUSampler(context.device, &sampler_info)));
    ...
    TTF_Font *font = static_cast<TTF_Font *>(check_error_ptr(TTF_OpenFont(font_filename, 50))); /* Preferably use a Monospaced font */
    if (!font) {
        running = false;
    }
    ...
    char str[] = "     \nSDL is cool";
    TTF_Text *text = static_cast<TTF_Text *>(check_error_ptr(TTF_CreateText(engine, font, str, 0)));
    ...
    while (running) {
        ...
        context.cmd_buf = static_cast<SDL_GPUCommandBuffer *>(check_error_ptr(SDL_AcquireGPUCommandBuffer(context.device)));
